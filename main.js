...
rhHypothesis.getMyAnnotations = async (articleUrl) => {
  const searchUrl = `search?user=${
    rhHypothesis.userProfile.userid
...
insertAnnotions = async (
  results,
  undoLastIndent = false,
  startingUid = ""
) => {
  ...
  if (results) {
    if (startingUid == "") startingUid = document.activeElement.id.slice(-9);
    let currentBlock = 0;
    for (var i = 0; i < results.length; i++) {
      ...
      let newBlock = await roam42.common.createBlock(
        startingUid,
        currentBlock,
        output
      );
      ...
      if (e.text != "") {
        //insert note
        var textOutput = await formatNoteBasedOnTemplate(
          nTemplate,
          e.text,
          e.context
        );
...
function getLinkAnnotations(link) {
  var userName = dotenv.hypothesis.HYPOTHESIS_USER;
  var password = dotenv.hypothesis.HYPOTHESIS_PASSWORD;

  return 
}

export default getLinkAnnotations;
...
(() => {
  ...
  rhHypothesis.insertAnnotions = async (
    results,
    undoLastIndent = false,
    startingUid = ""
  ) => {
  ...
  const initialize = async () => {
    ...
    if (rhHypothesis.userProfile.userid != null)
      rhHypothesis.initialized = true;
  };
